...
begin
  ...
  iNumPed := QDb.FieldByName('PK_Pedidos').AsInteger;
  ...
  try
    ...
    begin
      DDtPed := QDb.FieldByName('PddDatePed').AsDateTime;
      // Atribuir valores ao formulário
      frmManutencaoPedidos.DTDataPedido.Date := DDtPed;
      frmManutencaoPedidos.ECadPedido.Text := iNumPed.ToString;
    end
    else
    begin
...
var
  iNumPed: Integer;
begin
  iNumPed := DBGPrincipal.DataSource.DataSet.FieldByName('PK_Pedidos').AsInteger;
...
begin
  ...
  begin
    SQL.Text := 'SELECT * FROM C_PEDIDOS   '+
                ...
                '         LIKE :pPK_Pedidos'+
                '     ORDER BY PK_Pedidos  ';
    Params.ParamByName('pPK_Pedidos').AsString := '%' + iNumPed + '%';
    Open;
  end;
  if QDb.IsEmpty then
...
begin
  ...
  iNumPed := QDb.FieldByName('PK_Pedidos').AsString;
  ...
  begin
    ...
    ParamByName('pFK_Itens_Pedidos').AsString := iNumPed;
    Open;
  end;
end;
